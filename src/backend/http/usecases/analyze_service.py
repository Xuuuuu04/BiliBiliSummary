...
【架构位置】
位于 HTTP 层的用例子模块 (usecases/)，是视频分析业务逻辑的编排层。
介于 API 路由层和核心服务层之间，负责协调多个服务完成复杂业务流程。

【设计模式】
- 用例模式 (Use Case Pattern): 封装完整的业务流程
- 编排模式 (Orchestration Pattern): 协调多个服务的调用顺序
- 生成器模式: 使用 Iterator 实现流式响应

【主要功能】
1. 视频分析业务编排：
   - 协调 BilibiliService 获取视频数据
   - 协调 AIService 进行智能分析
   - 整合字幕、弹幕、评论、关键帧等多源数据

2. 两种分析模式：
   - analyze_video(): 同步模式，一次性返回完整结果
   - stream_analyze(): 流式模式，使用 SSE 逐步推送分析进度

【数据流程】
视频URL → 提取BV号 → 并行获取数据（字幕/弹幕/评论/统计/标签/合集/推荐）
      → 构建分析上下文 → 提取关键帧 → AI分析 → 返回结果

【输出格式】
同步模式: 标准JSON响应
流式模式: SSE (Server-Sent Events) 流式推送
